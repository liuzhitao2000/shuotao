...
YUI.add("st-feed-list", function(Y) {

    /* Any frequently used shortcuts, strings and constants */
    var Lang = Y.Lang;

    /* List class constructor */
    function FeedList(config) {
    ...
    /* Templates for any markup the widget uses. Usually includes {} tokens, which are replaced through Y.substitute */
		FeedList.APIHOST = "http://shuotao.me:8888/";
		FeedList.APIGETLIST = "newfeed2/";
		FeedList.MYNODE_TEMPLATE ='<a href="detail.html?tweetid={{tweet_id}}" data-tweet-id="{{tweet_id}}" class="tweet clear">' +
														'<div class="avatar">'+
		...
		FeedList.LOADING_TEMPLATE="<div id='list-loading'><img width='16' height='16' src='asset/loading.gif' /></div>";
    /* List extends the base Widget class */
    Y.extend(FeedList, Y.Widget, {
        ...
        renderUI : function() {
             ...
             */
						 ...
						 Y.one("#"+this.get("wrap")).insert(FeedList.LOADING_TEMPLATE,"after");
						 this._doRequest();	
        ...
        syncUI : function() {
            ...
            // this._uiSetAttrA(this.get("attrA"));
				...
				_getSuccess:function(o){
					...
					Y.log(j.status);
					var htmlarray = this._createHTML(j.feed),
							 html="",m,n;
					for(m=0,n=htmlarray.length;m<n;m++){
							html+=htmlarray[m]; 
					}
					Y.one('#feed-list-wrap').insert(html);
        ...
        _createUrl:function(){
					var baseUrl = FeedList.APIHOST+FeedList.APIGETLIST,url;
				...
				_createHTML : function(feed){
					return Y.Array.map(feed, function (tweet) {
							...
							Y.log(o);
							return Y.mustache(FeedList.MYNODE_TEMPLATE,o,FeedList.MYNODE_TEMPLATE_PARM);
					});
    ...
    Y.namespace("ST").FeedList = FeedList;
