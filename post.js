...
YUI.add("st-post", function(Y) {
    ...
    function Post(config) {
        Post.superclass.constructor.apply(this, arguments);
    }
    ...
    Post.ATTRS = {
        ...
        id : {
            value: "st-p-post"                     // The default value for attrA, used if the user does not set a value during construction.
    ...
    /* 
     * The HTML_`PARSER static constant is used if the Widget supports progressive enhancement, and is
    ...
    Post.BASE_TEMPLATE = "<div id={{id}} class='slideup in'><div>{{user_name}}</div><textarea>{{retweet}}</textarea></div>";

    /* MyWidget extends the base Widget class */
    Y.extend(Post, Y.Widget, {
        ...
        renderUI : function() {
             ...
             */
						 Y.log(Post.BASE_TEMPLATE);
             this._postWrap = Y.Node.create(Y.mustache(Post.BASE_TEMPLATE, {id:'post-wrap',
						 																																user_name:'akira',
																																						retweet:'hello'})); 
						 Y.one('body').append(this._postWrap);
        },
    ...
    Y.namespace("Plugin.ST").Post = Post;

}, "3.4.0", {requires:["widget", "substitute","mustache","node-load"]});
// END WRAPPER
